...
export async function executeDesktopSmoke(
  ...
  try {
    ...
    const browserSetup = [
      `localStorage.setItem('mimo-codex-open-tabs', ${JSON.stringify(JSON.stringify({
        openTabs: [{ sessionId: session.sessionId, title: 'Desktop Smoke', type: 'session' }],
        activeTabId: session.sessionId,
      }))})`,
      runtimeSelection
        ? `localStorage.setItem('mimo-codex-session-runtime', ${JSON.stringify(JSON.stringify({
          [session.sessionId]: runtimeSelection,
        }))})`
        : `localStorage.removeItem('mimo-codex-session-runtime')`,
    ]
