...
// Lazy-load adapter modules to avoid bundling optional dependencies (e.g. @whiskeysockets/baileys)
// into the main sidecar binary. These imports are only executed when the specific adapter
// API endpoint is actually called.
async function getWechatProtocol() {
  return await import('../../../adapters/wechat/protocol.js')
}

async function getWhatsAppProtocol() {
...
async function handleWechatAdaptersApi(req: Request, tail: string[]): Promise<Response> {
  const { pollWechatLoginWithQr, startWechatLoginWithQr, WECHAT_DEFAULT_BASE_URL } = await getWechatProtocol()
...
async function handleWhatsAppAdaptersApi(req: Request, tail: string[]): Promise<Response> {
  const { logoutWhatsAppAuth, pollWhatsAppLoginWithQr, startWhatsAppLoginWithQr } = await getWhatsAppProtocol()
  const { loadConfig } = await getCommonConfig()
