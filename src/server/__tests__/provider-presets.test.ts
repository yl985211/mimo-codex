...
describe('provider presets API', () => {
  ...
  test('configured presets can expose optional API key and promo metadata', () => {
    ...
    expect(zhipu?.promoText).toContain('mimo-codex')
  ...
  test('GET and PUT /api/providers/settings read and write mimo-codex settings.json', async () => {
    const initial = {
      env: {
    ...
    const updatedRaw = await fs.readFile(path.join(tmpDir, 'mimo-codex', 'settings.json'), 'utf-8')
