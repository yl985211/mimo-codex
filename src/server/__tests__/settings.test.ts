...
describe('Settings API', () => {
  ...
  it('GET /api/settings/cli-launcher should expose bundled launcher status', async () => {
    ...
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.command).toBe('mimo-codex')
...
describe('Models API', () => {
  ...
  it('GET /api/models/current should prefer mimo-codex managed model over global user model when provider is active', async () => {
  ...
  it('PUT /api/models/current should persist to mimo-codex managed settings when provider is active', async () => {
    const settingsSvc = new SettingsService()
    const providerSvc = new ProviderService()
    const provider = await providerSvc.addProvider({
      presetId: 'zhipuglm',
