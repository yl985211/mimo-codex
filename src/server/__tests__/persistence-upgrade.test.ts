...
describe('persistent storage upgrade migrations', () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mimo-codex-persistence-'))
  ...
  test('migrates legacy providers index and writes a backup before changing it', async () => {
    const ccHahaDir = path.join(tempDir, 'mimo-codex')
    ...
    expect(report.migratedEntries).toContain('mimo-codex/providers.json')
  ...
  test('imports legacy root providers config into mimo-codex storage without deleting the source', async () => {
    await fs.writeFile(
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).toContain('providers.json -> mimo-codex/providers.json')
    expect(report.migratedEntries).toContain('providers.json -> mimo-codex/settings.json')
    expect(JSON.parse(await fs.readFile(path.join(tempDir, 'providers.json'), 'utf-8'))).toMatchObject({
      version: 1,
      activeModel: 'legacy-sonnet',
    })

    const migrated = JSON.parse(await fs.readFile(path.join(tempDir, 'mimo-codex', 'providers.json'), 'utf-8')) as {
      activeId?: string | null
      providers?: Array<{
        id?: string
        presetId?: string
    ...
    const managedSettings = JSON.parse(await fs.readFile(path.join(tempDir, 'mimo-codex', 'settings.json'), 'utf-8')) as {
  ...
  test('does not overwrite current mimo-codex provider storage with a legacy root config', async () => {
    const ccHahaDir = path.join(tempDir, 'mimo-codex')
    await fs.mkdir(ccHahaDir, { recursive: true })
    await fs.writeFile(
      path.join(tempDir, 'providers.json'),
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).not.toContain('providers.json -> mimo-codex/providers.json')
  ...
  test('quarantines malformed managed settings instead of blocking startup', async () => {
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).toContain('mimo-codex/settings.json')
