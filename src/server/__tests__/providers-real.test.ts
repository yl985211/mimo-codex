 ...
 * 验证添加、激活、mimo-codex/settings.json 同步是否正确
 * (provider env 写到 ~/.claude/mimo-codex/settings.json，不污染原版 settings.json)
 */
...
describe('Real Provider Configs', () => {
  ...
  async function readCcHahaSettings(): Promise<Record<string, unknown>> {
    const raw = await fs.readFile(path.join(tmpDir, 'mimo-codex', 'settings.json'), 'utf-8')
  ...
  test('添加 MiniMax Provider 并激活 — 写入 mimo-codex/settings.json', async () => {
    ...
    console.log('✅ Provider 写入 mimo-codex/settings.json，原版 settings.json 未被污染')
  })

  test('切换 Provider — 更新 mimo-codex/settings.json', async () => {
    const minimax = await service.addProvider({
      presetId: 'minimax',
      name: 'MiniMax',
      baseUrl: 'https://api.minimaxi.com/anthropic',
    ...
    console.log('✅ 切换 Provider 成功，mimo-codex/settings.json 更新正确')
  })

  test('mimo-codex/settings.json 保留已有字段', async () => {
    // 预写一个有内容的 mimo-codex/settings.json（模拟用户已有配置）
    await fs.mkdir(path.join(tmpDir, 'mimo-codex'), { recursive: true })
    await fs.writeFile(
      path.join(tmpDir, 'mimo-codex', 'settings.json'),
      JSON.stringify({
    ...
    console.log('✅ mimo-codex/settings.json 已有字段全部保留')
  })
  ...
  test('providers.json 和 mimo-codex/settings.json 独立于 settings.json', async () => {
    ...
    // 验证 mimo-codex/settings.json 是 Haha 自己的
