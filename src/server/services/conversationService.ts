...
export class ConversationService {
  ...
  private async buildChildEnv(
    ...
    // from ~/.claude/mimo-codex/settings.json instead of stale process.env.
    //
    ...
    return {
      ...
      ...(explicitProviderEnv
        ...
        : {}),
      // "官方" 模式 (mimo-codex/settings.json 没 provider env) 下,把 CLI 标记为
  ...
  private shouldStripInheritedProviderEnv(providerId?: string | null): boolean {
    ...
    const ccHahaDir = path.join(configDir, 'mimo-codex')
   ...
   * 强制切 OAuth。
   *
   * 默认 (读不到 settings.json) 按"官方"处理 — 即使用户从未用过 mimo-codex
  ...
  private shouldMarkManagedOAuth(providerId?: string | null): boolean {
    ...
    const configDir =
      process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude')
    const settingsPath = path.join(configDir, 'mimo-codex', 'settings.json')
    try {
      const raw = fs.readFileSync(settingsPath, 'utf-8')
      const parsed = JSON.parse(raw) as { env?: Record<string, string> }
      const env = parsed.env ?? {}
  ...
  private resolveCliArgs(baseArgs: string[]): string[] {
    ...
    if (!launcher) {
      ...
      return [path.resolve(import.meta.dir, '../../../bin/mimo-codex'), ...baseArgs]
    }
  ...
  private buildStartupError(
    ...
    if (
      ...
      return new ConversationStartupError(
        'Desktop chat could not start because Claude CLI is not authenticated. Run `./bin/mimo-codex /login` or provide valid API credentials, then retry.',
        'CLI_AUTH_REQUIRED',
      )
    }
