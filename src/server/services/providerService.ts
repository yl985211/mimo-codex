 ...
 *
 * Storage: ~/.claude/mimo-codex/providers.json (lightweight index)
 * Active provider env vars written to ~/.claude/mimo-codex/settings.json
...
export class ProviderService {
  ...
  private getCcHahaDir(): string {
    return path.join(this.getConfigDir(), 'mimo-codex')
  }
   ...
   *  1. A mimo-codex provider is active → has auth
  ...
  async checkAuthStatus(): Promise<{
    hasAuth: boolean
    source: 'mimo-codex-provider' | 'openai-oauth' | 'original-settings' | 'env' | 'none'
    activeProvider?: string
  }> {
    // 1. Check mimo-codex active provider
    const index = await this.readIndex()
    if (index.activeId) {
      ...
      if (provider) {
        ...
        if (Object.values(authEnv).some(value => value.length > 0)) {
          return { hasAuth: true, source: 'mimo-codex-provider', activeProvider: provider.name }
        }
      }
    }
