...
export class DiagnosticsService {
  ...
  getLogDir(): string {
    return path.join(this.getConfigDir(), 'mimo-codex', 'diagnostics')
  }
  ...
  async recordEvent(input: DiagnosticEventInput): Promise<void> {
    ...
    // ~/.claude/mimo-codex/diagnostics. Tests that genuinely exercise diagnostics
  ...
  async exportBundle(): Promise<DiagnosticsExportResult> {
    ...
    const fileName = `mimo-codex-diagnostics-${timestamp}.tar.gz`
  ...
  private async buildProvidersSummary(): Promise<Record<string, unknown>> {
    const providerPath = path.join(this.getConfigDir(), 'mimo-codex', 'providers.json')
    try {
