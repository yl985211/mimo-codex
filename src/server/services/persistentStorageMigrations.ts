...
async function migrateLegacyRootProviders(
  ...
  try {
    ...
    if (errnoCode(error) !== 'ENOENT') {
      report.failures.push(`mimo-codex/providers.json: ${error instanceof Error ? error.message : String(error)}`)
      return
    }
  }
  ...
  try {
    ...
    report.migratedEntries.push('providers.json -> mimo-codex/providers.json')
    ...
    if (managedSettings) {
      ...
      report.migratedEntries.push('providers.json -> mimo-codex/settings.json')
    }
  } catch (error) {
    if (error instanceof SyntaxError) {
...
async function runPersistentStorageMigrations(configDir: string): Promise<MigrationReport> {
  ...
  await migrateJsonEntry(
    ...
    'mimo-codex/providers.json',
    report,
    migrateProvidersIndex,
  )
  await migrateJsonEntry(
    path.join(ccHahaDir, 'settings.json'),
    'mimo-codex/settings.json',
    report,
