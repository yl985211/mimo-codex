...
const DESKTOP_CLI_NAME = 'mimo-codex'
const DESKTOP_CLI_WINDOWS_LEGACY_EXE = `${DESKTOP_CLI_NAME}.exe`
const PATH_BLOCK_START = '# >>> MiMo-Codex PATH >>>'
const PATH_BLOCK_END = '# <<< MiMo-Codex PATH <<<'
...
if [[ ! -x "$SIDECAR" ]]; then
  echo "mimo-codex launcher could not find bundled sidecar: $SIDECAR" >&2
  exit 127
fi
...
export function buildWindowsLauncherWrapper(sourcePath: string) {
  ...
  return [
    ...
    '  echo mimo-codex launcher could not find bundled sidecar: %SIDECAR% 1>&2',
    '  exit /b 127',
    ')',
