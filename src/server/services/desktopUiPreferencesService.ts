...
const DEFAULT_PROFILE_SUBTITLE = 'github.com/yl985211/mimo-codex'
...
export class DesktopUiPreferencesService {
  ...
  private getPreferencesPath(): string {
    return path.join(this.getConfigDir(), 'mimo-codex', 'desktop-ui.json')
  }

  private getProfileDir(): string {
    return path.join(this.getConfigDir(), 'mimo-codex', 'profile')
  }
  ...
  private getProfileAvatarPath(avatarFile: string): string {
    ...
    return path.join(this.getConfigDir(), 'mimo-codex', normalized)
  }
  ...
  async readPreferences(): Promise<DesktopUiPreferencesReadResult> {
    ...
    const preferences = await readRecoverableJsonFile({
      filePath,
      label: 'mimo-codex desktop UI preferences',
