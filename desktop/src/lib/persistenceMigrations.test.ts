...
describe('desktop persistence migrations', () => {
  ...
  test('migrates legacy open-tab arrays into the current tab persistence shape', () => {
    window.localStorage.setItem('mimo-codex-open-tabs', JSON.stringify([
    ...
    expect(report.migratedKeys).toContain('mimo-codex-open-tabs')
    expect(JSON.parse(window.localStorage.getItem('mimo-codex-open-tabs') || '{}')).toEqual({
  ...
  test('filters stale session runtime selections without clearing unrelated keys', () => {
    ...
    window.localStorage.setItem('mimo-codex-session-runtime', JSON.stringify({
    ...
    expect(JSON.parse(window.localStorage.getItem('mimo-codex-session-runtime') || '{}')).toEqual({
      alsoGood: { providerId: 'provider-1', modelId: 'gpt-5.4' },
      good: { providerId: null, modelId: 'claude-sonnet' },
    })
  ...
  test('removes malformed known keys without throwing during startup', () => {
    window.localStorage.setItem('mimo-codex-open-tabs', '{"openTabs":')
    window.localStorage.setItem('mimo-codex-theme', 'sepia')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).toContain('mimo-codex-open-tabs')
    expect(report.migratedKeys).toContain('mimo-codex-theme')
    expect(window.localStorage.getItem('mimo-codex-open-tabs')).toBeNull()
    expect(window.localStorage.getItem('mimo-codex-theme')).toBeNull()
  })

  test('preserves the pure white theme as a valid persisted theme', () => {
    window.localStorage.setItem('mimo-codex-theme', 'white')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).not.toContain('mimo-codex-theme')
    expect(window.localStorage.getItem('mimo-codex-theme')).toBe('white')
  })

  test('preserves valid app zoom and removes invalid app zoom values', () => {
    window.localStorage.setItem('mimo-codex-app-zoom', '1.2')

    const validReport = runDesktopPersistenceMigrations()

    expect(validReport.migratedKeys).not.toContain('mimo-codex-app-zoom')
    expect(window.localStorage.getItem('mimo-codex-app-zoom')).toBe('1.2')

    window.localStorage.setItem('mimo-codex-app-zoom', '4')

    const invalidReport = runDesktopPersistenceMigrations()

    expect(invalidReport.migratedKeys).toContain('mimo-codex-app-zoom')
    expect(window.localStorage.getItem('mimo-codex-app-zoom')).toBeNull()
  })

  test('migrates the legacy UI zoom key into app zoom storage', () => {
    window.localStorage.setItem('mimo-codex-ui-zoom', '1.25')
    ...
    expect(window.localStorage.getItem('mimo-codex-app-zoom')).toBe('1.25')
    expect(window.localStorage.getItem('mimo-codex-ui-zoom')).toBeNull()
  })
  ...
  test('does not throw if storage reads and writes are blocked', () => {
    ...
    expect(report.migratedKeys).toEqual(expect.arrayContaining([
      'mimo-codex-open-tabs',
      'mimo-codex-session-runtime',
      'mimo-codex-theme',
      'mimo-codex-locale',
      'mimo-codex-app-zoom',
      DESKTOP_PERSISTENCE_VERSION_KEY,
