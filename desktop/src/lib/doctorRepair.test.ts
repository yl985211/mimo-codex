...
describe('doctorRepair', () => {
  ...
  it('clears only the safe desktop UI storage keys', () => {
    ...
    window.localStorage.setItem('mimo-codex-chat-history', 'preserve')
    window.localStorage.setItem('mimo-codex-provider-config', 'preserve')
    ...
    expect(window.localStorage.getItem('mimo-codex-chat-history')).toBe('preserve')
    expect(window.localStorage.getItem('mimo-codex-provider-config')).toBe('preserve')
  })
  ...
  it('keeps local repair successful when the server doctor endpoint is unavailable', async () => {
    ...
    expect(result.local.removedKeys).toContain('mimo-codex-theme')
