...
const PROJECT_ORDER_STORAGE_KEY = 'mimo-codex-sidebar-project-order'
const PROJECT_PINNED_STORAGE_KEY = 'mimo-codex-sidebar-pinned-projects'
const PROJECT_HIDDEN_STORAGE_KEY = 'mimo-codex-sidebar-hidden-projects'
const PROJECT_ORGANIZATION_STORAGE_KEY = 'mimo-codex-sidebar-project-organization'
const PROJECT_SORT_STORAGE_KEY = 'mimo-codex-sidebar-project-sort'
...
describe('Sidebar', () => {
  ...
  it('does not restore a hidden Windows drive root when creating a child project session', async () => {
    ...
    useSessionStore.setState({
      sessions: [
        makeSession('child-1', 'Child Session', 'D:\\workspace\\code\\mimo-codex', now),
      ],
    })
    useTabStore.setState({
    ...
    await waitFor(() => {
      expect(createSession).toHaveBeenCalledWith('D:\\workspace\\code\\mimo-codex')
    })
