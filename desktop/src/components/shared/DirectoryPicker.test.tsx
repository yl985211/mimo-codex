  ...
  it('does not duplicate the branch in the selected project chip', async () => {
    vi.mocked(sessionsApi.getRecentProjects).mockResolvedValue({
      projects: [{
        ...
        projectName: 'project',
        repoName: 'yl985211/OpenCutSkill',
        branch: 'main',
        isGit: true,
    ...
    const trigger = await waitFor(() => screen.getAllByRole('button', { name: /yl985211\/OpenCutSkill/ })[0])
    expect(trigger).toHaveTextContent('yl985211/OpenCutSkill')
