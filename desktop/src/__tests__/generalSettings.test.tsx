...
describe('Settings > General tab', () => {
  beforeEach(() => {
    ...
    tauriDialogMock.open.mockResolvedValue('/Users/test/mimo-codex-data')
    ...
    useSettingsStore.setState({
      ...
      traceCapture: { enabled: true, storageDir: '/Users/test/.claude/mimo-codex/traces' },
      ...
      appMode: {
        mode: 'default',
        portableDir: null,
        defaultPortableDir: '/Applications/MiMo-Codex/CLAUDE_CONFIG_DIR',
      ...
      setAppMode: vi.fn().mockImplementation(async (mode: AppMode, portableDir?: string | null) => {
        useSettingsStore.setState({
          appMode: {
            mode,
            portableDir: mode === 'portable' ? portableDir ?? '/Applications/MiMo-Codex/CLAUDE_CONFIG_DIR' : null,
            defaultPortableDir: '/Applications/MiMo-Codex/CLAUDE_CONFIG_DIR',
            activeConfigDir: mode === 'portable' ? portableDir ?? '/Applications/MiMo-Codex/CLAUDE_CONFIG_DIR' : null,
  ...
  it('lets desktop users choose a portable data directory and relaunch immediately', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Portable data directory')).toHaveValue('/Users/test/mimo-codex-data')
    })
    ...
    await waitFor(() => {
      expect(useSettingsStore.getState().setAppMode).toHaveBeenCalledWith('portable', '/Users/test/mimo-codex-data')
  ...
  it('switches back to the system directory without deleting portable data', async () => {
    useSettingsStore.setState({
      appMode: {
        mode: 'portable',
        portableDir: '/Users/test/mimo-codex-data',
        defaultPortableDir: '/Applications/MiMo-Codex/CLAUDE_CONFIG_DIR',
        activeConfigDir: '/Users/test/mimo-codex-data',
  ...
  it('validates portable directory input and lets users reset to the app-side folder', async () => {
    ...
    expect(input).toHaveValue('/Applications/MiMo-Codex/CLAUDE_CONFIG_DIR')
  ...
  it('requests native notification permission when desktop notifications are enabled', async () => {
    ...
    expect(desktopNotificationsMock.notifyDesktop).toHaveBeenCalledWith({
      title: 'MiMo-Codex notifications are enabled',
...
describe('Settings > About tab', () => {
  beforeEach(() => {
    ...
    useUpdateStore.setState({
      ...
      releaseNotes: '# MiMo-Codex v0.1.5\n\n- Fixed updater rendering\n- Added markdown support',
      progressPercent: 0,
      downloadedBytes: 0,
      totalBytes: null,
      error: null,
  ...
  it('renders release notes with markdown formatting', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'MiMo-Codex v0.1.5' })).toBeInTheDocument()
