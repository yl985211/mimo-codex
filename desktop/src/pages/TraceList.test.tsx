...
const traceList: TraceSessionList = {
  total: 1,
  storageDir: '/tmp/mimo-codex/traces',
  settings: {
    enabled: true,
    storageDir: '/tmp/mimo-codex/traces',
  },
...
describe('TraceList', () => {
  ...
  it('renders rows with title, model chips, failure count and metrics', async () => {
    ...
    expect(screen.getByText('/tmp/mimo-codex/traces')).toBeInTheDocument()
  ...
  it('shows the paused badge when capture is disabled', async () => {
    vi.mocked(tracesApi.list).mockResolvedValue({
      ...traceList,
      settings: { enabled: false, storageDir: '/tmp/mimo-codex/traces' },
    })
