...
describe('ActivitySettings', () => {
  ...
  it('renders summary metrics and a GitHub-style trailing heatmap without future days', async () => {
    ...
    expect(screen.getByText('mimo-codex')).toBeInTheDocument()
    expect(screen.getByAltText('mimo-codex avatar')).toHaveAttribute('src', '/app-icon.png')
    expect(screen.getByAltText('mimo-codex avatar')).toHaveClass('scale-[1.28]')
    expect(screen.getByRole('link', { name: 'github.com/yl985211/mimo-codex' })).toHaveAttribute(
      'href',
      'https://github.com/yl985211/mimo-codex',
    )
