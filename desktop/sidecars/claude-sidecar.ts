...
/**
 * Set CLAUDE_CONFIG_DIR to a .mimo-codex folder next to the executable.
 * This makes the app portable and separates it from Claude Code Haha.
 */
function initConfigDir(): void {
  ...
  if (execPath.endsWith('.exe')) {
    ...
    exeDir = dirname(process.resourcesPath)
  } else {
    exeDir = process.cwd()
  }

  // Create .mimo-codex folder next to exe
  const configDir = join(exeDir, '.mimo-codex')
  ...
  // Set environment variable so all services use this directory
  process.env.CLAUDE_CONFIG_DIR = configDir
}

// Initialize config dir BEFORE importing any other modules
initConfigDir()
