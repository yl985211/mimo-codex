...
// Lazy-load WhatsApp modules to avoid bundling @whiskeysockets/baileys in sidecar
let baileysModule: any = null
let sessionModule: any = null
let mediaModule: any = null
...
async function getMedia() {
  if (!mediaModule) {
    mediaModule = await import('./media.js')
  }
  return mediaModule
}
...
if (!session.hasWhatsAppAuth(authDir)) {
...
function extractText(raw: any): string {
  const baileys = baileysModule || { normalizeMessageContent: (m: any) => m }
  const msg = baileys.normalizeMessageContent(raw)
  if (!msg) return ''
...
async function startSocket(): Promise<void> {
  const session = await getSession()
  const MediaService = (await getMedia()).WhatsAppMediaService

  if (reconnectTimer) {
    clearTimeout(reconnectTimer)
    reconnectTimer = null
  }
  sock = await session.createWhatsAppSocket({ authDir })
  media = new MediaService(sock, attachmentStore)
...
process.on('SIGINT', async () => {
  ...
  const session = await getSession()
  session.closeWhatsAppSocket(sock, 'SIGINT')
  bridge.destroy()
  dedup.destroy()
  process.exit(0)
})
