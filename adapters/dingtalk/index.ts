...
// Lazy-load dingtalk-stream to avoid bundling in sidecar
let DingTalkModule: any = null
async function getDingTalk() {
  if (!DingTalkModule) {
    DingTalkModule = await import('dingtalk-stream')
  }
...
async function start(): Promise<void> {
  const DingTalk = await getDingTalk()
  const client = new DingTalk.DWClient({
    clientId: config.dingtalk.clientId,
  ...
  client.registerCallbackListener(DingTalk.TOPIC_ROBOT, async (res: any) => {
  ...
  client.registerCallbackListener(DingTalk.TOPIC_CARD, async (res: any) => {
    const messageId = res.headers?.messageId
    if (messageId) {
      client.socketCallBackResponse(messageId, { success: true })
