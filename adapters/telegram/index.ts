...
// Lazy-load grammY to avoid bundling in sidecar
let GrammyModule: any = null
async function getGrammy() {
  if (!GrammyModule) {
...
const Grammy = await getGrammy()
const bot = new Grammy.Bot(config.telegram.botToken)
...
const { TelegramMediaService } = await import('./media.js')
