...
```bash
./bin/mimo-codex                          # Interactive TUI mode
./bin/mimo-codex -p "your prompt here"    # Headless mode
./bin/mimo-codex --help                   # Show all options
```
...
```bash
CLAUDE_CODE_FORCE_RECOVERY_CLI=1 ./bin/mimo-codex
```
